...
import { Divider, Checkbox, Button, Row, Col, Tooltip, Modal, Select, Card,Drawer } from 'antd'
import styles from './style.less'
import echarts from 'echarts'
import GridSet from './tools'
...
class Ol_mapproxy_tool extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      wmsSource: new TileWMS({
        url: 'http://localhost:8081/geoserver/map/wms',
        params: { 'LAYERS': 'map:china', 'TILED': true },
  ...
  initMap = () => {
    const { raster, arcgisLayer, imagesLayer, digitalLayer, china,gridLayer } = this.state
    /** 先设置图层的显示隐藏 */
  ...
  //渲染页面html模块
  render() {
    const { map, showPopup, popHtml, drawVisible } = this.state
    return (
...
export default Ol_mapproxy_tool
